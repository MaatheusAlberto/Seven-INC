...
const EmployeeEdit = () => {
  ...
  const submitDados = (values) => {
    ...
    axios.put(`http://localhost:8080/api/employee/${id}`, payload)
    .then(response => {
      console.log('Success:', response.data);
      setToastOpen({open: true, text: 'Atualização do cadastro realizada com sucesso!!', color:'1'})
      setIsLoading(false)
    })
    .catch(error => {
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className={classes.space}>
                  <Button type="submit" variant='contained' color="primary" disabled={isLoading}>
                    {isLoading ? 'Loading...' :'Salvar Alteração'}
                  </Button>
                </div>
              </form>
            )
